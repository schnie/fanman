...
import { byeLoads } from '../domain/byes'
...
export function Roster({ picks, players, summary, slots }: {
  ...
  // One pass for both: the lineup and the bye loads read the same roster, and
  // splitting them meant building the id map and the won-pick list twice.
  //
  // What comes out of the bye pass is only `shortAt`: which positions each
  // week is actually short at, so a row can flag the player it's about. Asked
  // with the player's own position, because a week can be short at back and
  // perfectly fine at receiver, and the receiver with three bodies behind him
  // is not the problem.
  const { lineup, shortAt } = useMemo(() => {
    const byId = new Map(players.map((p) => [p.id, p]))
    const won = wonPicksFrom(picks)
    const shortAt = new Map<number, Set<string>>()
    for (const load of byeLoads(won, byId, slots)) {
      if (load.holes > 0) shortAt.set(load.week, new Set(load.uncoveredPositions))
    }
    return { lineup: buildLineup(won, byId, slots), shortAt }
  }, [picks, players, slots])
